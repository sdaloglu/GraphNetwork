...
parser = argparse.ArgumentParser(description='Training script for different simulations.')
...
for epoch in tqdm(range(epochs)):
  ...
  for test_batch in test_loader:    # We need multiple batches per epoch to plot y=x
    ...
    # Recording the true ouptut of the test batch
    true_acceleration = test_batch.y.cpu().numpy()
    current_message['true_acceleration'] = true_acceleration
...
pkl.dump(recorded_models, open('models/pruned_models_{}_{}.pkl'.format(title, regularizer), 'wb'))

# Save the messages
pkl.dump(messages_over_time, open('models/pruned_messages_{}_{}.pkl'.format(title, regularizer), 'wb'))
