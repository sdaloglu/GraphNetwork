...
# Load model pkl file
recorded_models = pkl.load(open(f'models/{title[:2]}/pruned_models_{title}_{regularizer}.pkl', 'rb'))
...
for i in t(range(0, len(messages_over_time), 1)):
    ...
    msg_importance = np.mean(msg_array**2, axis=0)
...
directory = 'data'
file_name = f"pruned_kl_{title}.html"
