...
messages_over_time = pkl.load(open('models/pruned_messages_{}_{}.pkl'.format(title, regularizer), 'rb'))

# Load model pkl file
recorded_models = pkl.load(open('models/pruned_models_{}_{}.pkl'.format(title, regularizer), 'rb'))
...
for i in t(range(0, len(messages_over_time), 1)):
    ...
    for j in range(dim):
        ...
        # Place R2 value 
        ax.text(0.5, 0.95, f'$R^2$: {r2_values:.2f}', ha='center', va='top', transform=ax.transAxes, fontsize=12, color='red')
...
directory = 'data'
file_name = f"pruned_stand_{title}.html"
